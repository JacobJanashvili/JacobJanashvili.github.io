...
let receivedMovieId = sessionStorage.getItem("movieId");
let movie_Img = document.getElementsByClassName("movie-img");
console.log(document.getElementsByClassName("movie-img"));
let movieIdUrl =
  base_api_url +
  `movie/${receivedMovieId}?api_key=${api_key}&append_to_response=videos`;
getMovieID();
function getMovieID() {
  fetch(movieIdUrl)
    .then((res) => res.json())
    .then((data) => {
      ...
      let splitBudget = budget.toLocaleString();
      row_movie.classList.add("row_movie");
      row_movie.innerHTML = `
      <div class="wrapper">
      <img
      class="movie-img" src="https://image.tmdb.org/t/p/w300${
        videos ? poster_path : backdrop_path
      }"
      alt=""
    />
    <div class="video">
  ...
  <div class="row_movie_overview">
    <div class="description">
      <div class="description_column">
        ...
        <p>Budget: ${splitNumber(budget)}</p>
        <p>Revenue: ${splitNumber(revenue)}</p>
      </div>
    </div>
    <div class="overview">
      <h3>Overview:</h3>
...
function genreFunction(object) {
  for (let i = 0; i < object.length; i++) {
    ...
    let returnStatement = "Information Not Found";
